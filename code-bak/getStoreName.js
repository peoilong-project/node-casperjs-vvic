...
if (args.length >= 1) { // ("http://")>=0 || args2.indexOf("https://")>=0){
    store_name = args[0];
    get_shop_url = "http://www.vvic.com/gz/shops/search.html?q=" + store_name;
    shop_tUrl = args[1];
    console.log(get_shop_url);
...
casper.open(get_shop_url)
    .then(function() {
        console.log("已获取到店铺链接");
        var postData = {};
        var isClose = this.fetchText(".empty-tip");
        console.log(isClose);
        if (isClose.indexOf("没有找到相关档口") != -1) {
            postData.seller_name = store_name;
            console.log(postData.seller_name);
        ...
        shop_url_list = this.evaluate(function() {
            var store_url = getStoreUrl();
            return store_url;
        });
        console.log(shop_url_list);
        casper.start().each(shop_url_list, function(self, link) {
                self.thenOpen(link, function() {
                    var shop_taobao_url = casper.evaluate(function() {
                        var item = getTaobao_url();
                        return item;
                    });
                    if (shop_tUrl.indexOf(shop_taobao_url) == -1) {
                        console.log("no");
                        casper.die();
                    } else {
                        shop_url = link;
        ...
        casper.open(shop_url)
            ...
            .waitForText("后一页", function() { //目前在第一页
